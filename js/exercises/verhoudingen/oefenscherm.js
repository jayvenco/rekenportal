// exercises/verhoudingen/oefenscherm.js
// -----------------------------------------------------------------------------
// Het oefenscherm van verhoudingen: toont recept-/schaalvragen één voor één,
// verwerkt antwoorden via cijfertoetsenbord, geeft feedback (groen/oranje/rood)
// met voortgangscirkels.
// -----------------------------------------------------------------------------
...
const EXERCISE_ID = "verhoudingen";
...
export function startOefensessie(container, instellingen, opKlaar) {
  ...
  // --- Invoergebied (cijfertoetsenbord) ---
  ...
  function toonOpgave() {
    ...
    // Altijd cijfertoetsenbord (cijferinvoer), zoals vereist
    opruimHuidigeInvoer = bouwCijferInvoer(invoerVlak, verwerkAntwoord, () => bezigMetFeedback);
  }
  ...
  async function verwerkAntwoord(antwoordVanKind) {
    ...
    try {
      ...
      if (isGoed) {
        ...
        // Tweede poging ook fout: toon het juiste antwoord, ga door
        bezigMetFeedback = true;
        await recordAnswer({
          exerciseId: EXERCISE_ID,
          correct: false,
