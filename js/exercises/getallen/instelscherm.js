// exercises/getallen/instelscherm.js
// -----------------------------------------------------------------------------
// Instelscherm voor negatieve getallen + kommagetallen.
// -----------------------------------------------------------------------------

import { getInstellingen, saveInstellingen } from "../../storage.js";

const ALLE_TYPES = [
  { id: "negatief-plus", label: "Negatief optellen", voorbeeld: "-5 + 3 = ?" },
  { id: "negatief-min", label: "Negatief aftrekken", voorbeeld: "7 - (-3) = ?" },
  { id: "komma-keer", label: "Komma × heel", voorbeeld: "0,5 × 6 = ?" },
  { id: "komma-delen", label: "Komma ÷ komma", voorbeeld: "2,4 ÷ 0,6 = ?" },
];
...
export async function bouwInstelscherm(container, opStarten) {
  const opgeslagen = (await getInstellingen("getallen")) || {};
  const instellingen = {
  ...
  titel.textContent = "Negatief + kommagetallen";
  kaart.appendChild(titel);

  const uitleg = document.createElement("p");
  uitleg.textContent = "Oefen met negatieve getallen en kommagetallen. Je kunt een komma (,) of punt (.) gebruiken.";
  ...
  startKnop.addEventListener("click", async () => {
    ...
    await saveInstellingen("getallen", instellingen);
