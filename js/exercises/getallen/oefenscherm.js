// exercises/getallen/oefenscherm.js
// -----------------------------------------------------------------------------
// Het oefenscherm voor negatieve getallen + kommagetallen.
// Cijferinvoer met minteken en komma/punt.
// -----------------------------------------------------------------------------
...
const EXERCISE_ID = "getallen";

/**
 * Start de getallen-oefensessie.
...
export function startOefensessie(container, instellingen, opKlaar) {
  ...
  // --- Cijferinvoer met minteken en komma ---
  function bouwGetallenInvoer(maxCijfers = 5) {
    invoerVlak.innerHTML = "";
    let huidigeWaarde = "";
    let mintekenActief = false;
    ...
    function toonWaarde() {
      const voorvoegsel = mintekenActief ? "-" : "";
      scherm.textContent = voorvoegsel + huidigeWaarde;
    }
    ...
    // Minteken-knop (toggles)
    const minKnop = document.createElement("button");
    minKnop.type = "button";
    minKnop.className = "cijfer-toets";
    minKnop.textContent = "−";
    minKnop.setAttribute("aria-label", "Minteken aan/uit");
    minKnop.style.background = "#e8735a";
    minKnop.style.color = "#ffffff";
    minKnop.addEventListener("click", () => {
      mintekenActief = !mintekenActief;
      minKnop.style.outline = mintekenActief ? "3px solid #1f2937" : "none";
      toonWaarde();
    });
    ...
    bevestigKnop.addEventListener("click", () => {
      if (huidigeWaarde === "" && !mintekenActief) return;
      let waardeStr = huidigeWaarde.replace(",", ".");
      if (mintekenActief) waardeStr = "-" + waardeStr;
      if (waardeStr === "" || waardeStr === "-") return;
      verwerkAntwoord(Number(waardeStr));
    });
    ...
    function toetsHandler(gebeurtenis) {
      ...
      if (gebeurtenis.key === "-" || gebeurtenis.key === "−") {
        mintekenActief = !mintekenActief;
        minKnop.style.outline = mintekenActief ? "3px solid #1f2937" : "none";
        toonWaarde();
      } else if (/^[0-9]$/.test(gebeurtenis.key)) {
  ...
  function toonOpgave() {
    ...
    opruimHuidigeInvoer = bouwGetallenInvoer(6);
  }
