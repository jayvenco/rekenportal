// exercises/statistiek/oefenscherm.js
// -----------------------------------------------------------------------------
// Het oefenscherm van de statistiek-module: toont grafieken, tabellen,
// datasets en vraagt antwoorden. Altijd cijferinvoer (geen meerkeuze).
// 2 pogingen, groen/oranje/rood cirkels.
// -----------------------------------------------------------------------------
...
export function startOefensessie(container, instellingen, opKlaar) {
  ...
  // --- Vraagvlak (voor grafieken/tabellen/datasets) ---
  ...
  function bijwerkenVoortgang() {
    voortgangTekst.textContent =
      `Opgave ${Math.min(opgaveIndex + 1, instellingen.aantalOpgaven)} van ${instellingen.aantalOpgaven} — ${aantalGoedTotaal} goed`;
  }
  ...
  function toonOpgave() {
    ...
    // Render de vraag: htmlVraag-opgaven (grafieken/tabellen) krijgen
    // de volledige innerHTML; tekst-opgaven worden in een span gewikkeld.
    if (huidigeOpgave.htmlVraag) {
      vraagVlak.innerHTML = huidigeOpgave.vraagTekst;
    } else {
      vraagVlak.innerHTML = `<span class="getal-groot">${huidigeOpgave.vraagTekst}</span>`;
    }

    // Altijd cijferinvoer (geen meerkeuze — antwoorden zijn hele getallen)
    opruimHuidigeInvoer = bouwCijferInvoer(
      invoerVlak,
      verwerkAntwoord,
      () => bezigMetFeedback,
      3 // max 3 cijfers (antwoorden tot 999)
    );
  }
  ...
  async function verwerkAntwoord(antwoordVanKind) {
    ...
    try {
      ...
      if (isGoed) {
        ...
        // Opnieuw cijferinvoer voor tweede poging
        opruimHuidigeInvoer = bouwCijferInvoer(
          invoerVlak,
          verwerkAntwoord,
          () => bezigMetFeedback,
  ...
  function toonEindscherm() {
    ...
    const percentageGoedVoorTitel =
      (aantalGoedTotaal / instellingen.aantalOpgaven) * 100;
    if (percentageGoedVoorTitel === 100) toonPerfecteScoreAnimatie();
    titel.textContent =
      percentageGoedVoorTitel >= 70
        ? "Goed gedaan!"
        : "Bijna! Nog even oefenen.";
    kaart.appendChild(titel);

    const raketEind = toonEindAnimatie(
      kaart,
      (aantalGoedTotaal / instellingen.aantalOpgaven) * 100
    );
